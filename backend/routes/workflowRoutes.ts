// D:/code/ai-flow-master/backend/routes/workflowRoutes.ts (修复后)
import express, { Router, Request, Response } from 'express';

// *** 修正导入：从 types 导入共享类型，并恢复使用 .js 扩展名 ***
import type { AIModule, Port } from '../../types/ai_module_models.js'; // <-- 恢复 .js
import type { Connection } from '../../types/Connection.js';       // <-- 恢复 .js
import type { CanvasItem } from '../../types/CanvasItem.js';         // <-- 恢复 .js
...
interface StoredWorkflow {
    ...
    name: string;
    description: string;
    canvasItems: CanvasItem[]; // 使用从 types 导入的 CanvasItem
...
const availableAIModules: AIModule[] = [
    {
        id: 'text-input', name: '文本输入', category: '输入/输出', inputs: [], outputs: [{ id: 'text_out', label: '文本', type: 'string' }],
        configSchema: { 'input_text': { type: 'textarea', label: '输入文本', defaultValue: '默认文本' } },
        defaultConfig: { 'input_text': '默认文本' }, icon: '📄'
    },
    {
        id: 'text-output', name: '文本输出', category: '输入/输出', inputs: [{ id: 'text_in', label: '文本', type: 'string' }], outputs: [], icon: '📤'
    },
    {
        id: 'uppercase', name: '转大写', category: '文本处理', inputs: [{ id: 'text_in', label: '输入文本', type: 'string' }], outputs: [{ id: 'text_out', label: '输出文本', type: 'string' }], icon: '⬆️'
    },
];

// GET /api/workflows - 获取所有工作流 (基本信息)
...
router.post('/workflows', (req: Request, res: Response) => {
    ...
    // TODO: Add validation for canvasItems and connections structure if received
    const newWorkflow: StoredWorkflow = {
        id: nextWorkflowId++,
        name,
        description: description || '',
        canvasItems: Array.isArray(canvasItems) ? canvasItems as CanvasItem[] : [], // Basic validation
        connections: Array.isArray(connections) ? connections as Connection[] : [], // Basic validation
    };
    workflows.push(newWorkflow);
    console.log('后台: 工作流已创建:', newWorkflow.id, newWorkflow.name);
    // Return the complete newly created workflow object
    res.status(201).json(newWorkflow);
});

// GET /api/workflows/:id - 获取指定 ID 的工作流完整数据
router.get('/workflows/:id', (req: Request, res: Response) => {
    ...
    res.status(200).json(workflow); // Return the full StoredWorkflow
});

// PUT /api/workflows/:id - 更新指定 ID 的工作流
router.put('/workflows/:id', (req: Request, res: Response) => {
    ...
    // Simple type checking, real app needs robust validation (e.g., using zod)
    ...
    // TODO: Add validation for the structure within canvasItems and connections arrays

    const updatedWorkflow: StoredWorkflow = {
        ...workflows[index], // Start with existing workflow
    ...
    res.status(200).json(updatedWorkflow); // Return the full updated workflow
});

// DELETE /api/workflows/:id - 删除指定 ID 的工作流
router.delete('/workflows/:id', (req: Request, res: Response) => {
    ...
    if (workflows.length < initialLength) {
        ...
        // Use 404 for resource not found on delete attempt
